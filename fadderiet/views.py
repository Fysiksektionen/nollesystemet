...
from fadderiet.helper_views import MenuBaseView, MultipleObjectsUpdateView, MenuBaseMixin
...
class ProfilePageView(LoginRequiredMixin, MenuBaseMixin, MultipleObjectsUpdateView):
    menu_item_info = MenuBaseView.menu_item_info
    menu_items = MenuBaseView.menu_items

    model_list = [apps.get_model(settings.AUTH_USER_MODEL), apps.get_model(settings.USER_PROFILE_MODEL)]
    fields_list = [['username', 'email'],
                   ['first_name', 'last_name', 'kth_id', 'phone_number', 'food_preference',
                    'contact_name', 'contact_relation', 'contact_phone_number']]
    ...
    def get_context_data(self, **kwargs):
        context = MenuBaseMixin.get_context_data(self, **kwargs)
        context.update(MultipleObjectsUpdateView.get_context_data(self, **kwargs))
        return context
