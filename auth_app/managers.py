...
class UserProfileManager(BaseUserManager):
    ...
    def _create_user(self, username, first_name, last_name, password, **extra_fields):
        if not username:
            raise ValueError('The given username must be set')
        username = apps.get_model('auth_app', 'AuthUser').normalize_username(username)

        is_staff = extra_fields.pop('is_staff')
        is_superuser = extra_fields.pop('is_superuser')

        auth_user = apps.get_model('auth_app', 'AuthUser')(username=username, is_staff=is_staff, is_superuser=is_superuser)
